...
const CodeSearch = ({ classNameSelect }: CodeSearchProps) => {
  ...
  const [codeData, setCodeData] = useState<{ [key: string]: any }>({})

  const [codeDataOptions, setCodeDataOptions] = useState([])
  ...
  useEffect(() => {
    ;(async () => {
      const data = await window.electronAPI.getJsonData('codeData')
        ...
        <div className='w-1/5 flex flex-col min-w-[125px]' >
          <Select value={codeCategory} onValueChange={(e) => onCategoryChange(e)}  >
            ...
            <SelectContent className=' '>
              {codeCategoryOptions && codeCategoryOptions.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
